...
function CatalogContent({ canal, title, subtitle }: CatalogViewProps) {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [cats, prods] = await Promise.all([
        getActiveCategories(),
        getActiveProducts(canal, selectedCategory || undefined),
      ]);
      setCategories(cats);
      setProducts(prods);
      setLoading(false);
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-8">
          {loading ? (
            ...
            <ProductGrid products={products} canal={canal} />
          )}
        </div>
      </main>
      <Footer />
...
/** Catálogo de un canal. Lo usan /catalogo (mayorista) y /tienda (minorista). */
